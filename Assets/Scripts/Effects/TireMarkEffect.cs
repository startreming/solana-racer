...
namespace Effects
{
    ...
    public class TireMarkEffect : MonoBehaviour
    {
        Transform tr;
        Mesh mesh;
        int[] tris;
        ...
        private float markHeight = -0.05f;
        public int markLength = 80;
        public float markGap = 0.1f;
        public float lifeTime = 5.0f;
        public float fadeRate = 1.0f;
        public float markOffset = 0.0f;
        public float wheelWidth = 3f;
        public float flippedSideFactor = 0.1f;
        public float rotationRate = 0.9f;
        ...
        public void SetTire(Transform transform)
        {
            tire = transform;
        }
        ...
        void Update()
        {
            ...
            {
                grounded = kartController.Grounded;
                sliding = kartController.Sliding;
            }
            ...
            if (grounded) {
                ...
                {
                    continuousSlide = 0.0f; //surfaceProps.alwaysSlide ? Mathf.Min(0.9f, Mathf.Abs(wheel.rotationRate * 0.01f)) : 0.0f;
                }
                else {
            ...
            if (grounded && (sliding || continuousSlide > 0)) {
                prevSurface = curSurface;
                curSurface = grounded ? surfaceProps : defaultSurface;
                ...
                if (curMark != null) {
                    ...
                    Vector3 contactPoint = tire.position;
                    
                    Vector3 pointDir = Vector3.ProjectOnPlane(Quaternion.AngleAxis(90f, contactNormal) *tr.forward, contactNormal).normalized * wheelWidth * -0.5f;

                    leftPoint = curMarkTr.InverseTransformPoint(contactPoint + pointDir * (flippedSideFactor * Mathf.Sign(rotationRate) - markOffset * flippedSideFactor) + contactNormal * markHeight);
                    rightPoint = curMarkTr.InverseTransformPoint(contactPoint - pointDir * (flippedSideFactor * Mathf.Sign(rotationRate) + markOffset * flippedSideFactor) + contactNormal * markHeight);
                }
            }
            ...
            if (curEdge < markLength && creatingMark && kartController.Grounded) {
        ...
        private float MaxAbs(params float[] nums) {
            float result = 0;
        ...
        void UpdateMark() {
            ...
            var currentRotate = kartController.CurrentRotate;
            var forward = -kartModel.transform.right;
            
            float leftAhead = 0f, rightAhead = 0f;
            var steeringPower = currentRotate * 0.015f;

            if (currentRotate > 0f)
                rightAhead = Mathf.Clamp(steeringPower, 0.15f, 0.4f);
            else
                leftAhead = Mathf.Clamp(-steeringPower, 0.15f, 0.4f);
            
            var leftOffset = forward * leftAhead;
            var rightOffset = forward * rightAhead;

            if (gapDelay == 0)
            {
                float maxSlide = 1f;//Mathf.Clamp01(Mathf.Abs(MaxAbs(sliding ? 1.0f : 0.0f, continuousSlide)));
                float alpha = 1f;//(curEdge < markLength - 2 && curEdge > 5 ? 1.0f : 0.0f) * Random.Range(maxSlide * 0.7f, maxSlide);
                gapDelay = markGap;
                curEdge += 2;

                verts[curEdge] = leftPoint + leftOffset;
                verts[curEdge + 1] = rightPoint + rightOffset;
    ...
    public class TireMark : MonoBehaviour
    {
        ...
        void Update() {
            if (fading) {
                ...
                else if (lifeTime <= 0f && lifeTime > -1f) {
                    fading = true;
                }
            }
        }
